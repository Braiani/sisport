                                ...
                                @if ($dataType->slug == 'portarias')
                                    ...
                                    <div class="form-group col-sm-3">
                                        <div class="row">
                                            ...
                                            <a href="{{ route('voyager.pessoas.show', $selected_value->id) }}">{{ $selected_value->nome }}</a>
                                        </div>
                                        <div class="row">
                                ...
                                @foreach ($pessoasSelecionadas->pessoas as $pessoasSelecionada)
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4 form-group">
                                            ...
                                            <input id="dt_rel_{{$pessoasSelecionada->id}}" type="date"
                                                   name="data_relatorio[{{$pessoasSelecionada->id}}]"
                                                   class="form-control dt_relatorio"
                                                   value="{{ $pessoasSelecionada->pivot->data_relatorio ? $pessoasSelecionada->pivot->data_relatorio->format('Y-m-d') : ''  }}">
                                        </div>
                                        <div class="col-md-4 form-group">
                                            <label>Entregou o relatório?</label>
                                            <input id="en_rel_{{$pessoasSelecionada->id}}" type="text"
                                                   name="entregou_relatorio[{{$pessoasSelecionada->id}}]"
                                                   class="form-control ent_relatorio"
                                                   value="{{ $pessoasSelecionada->pivot->entregou_relatorio }}">
                                        </div>
                                        <div class="col-md-4 form-group">
                                            <label>Declaração emitida?</label>
                                            <input id="declaracao_{{$pessoasSelecionada->id}}" type="text"
                                                   name="declaracao[{{$pessoasSelecionada->id}}]"
                                                   class="form-control declaracao"
                                                   value="{{ $pessoasSelecionada->pivot->declaracao }}">
                                        </div>
                                        <hr>
                                    </div>
                                @endforeach
                ...
                $('#membros').on('change', function () {
                    ...
                    $('#membros option:selected').each(function () {
                        $('#info_adicionais').append(
                            ...
                            '<div class="col-md-4 form-group">' +
                            '<label>Data para entregar relatório:</label>' +
                            '<input id="dt_rel_' + $(this).val() + '" type="date" name="data_relatorio[' + $(this).val() + ']" class="form-control dt_relatorio">' +
                            '</div>' +
                            '<div class="col-md-4 form-group">' +
                            '<label>Entregou o relatório?</label>' +
                            '<input id="en_rel_' + $(this).val() + '" type="text" name="entregou_relatorio[' + $(this).val() + ']" class="form-control ent_relatorio">' +
                            '</div>' +
                            '<div class="col-md-4 form-group">' +
                            '<label>Declaração emitida?</label>' +
                            '<input id="declaracao_' + $(this).val() + '" type="text" name="declaracao[' + $(this).val() + ']" class="form-control declaracao">' +
                            '</div>' +
                            '<hr>'
                        );
                    });
